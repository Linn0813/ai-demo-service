...
from domain.test_case.content_extractor import ContentExtractor
from shared.debug_recorder import record_ai_debug
from domain.test_case.heuristic_extractor import HeuristicExtractor
from domain.test_case.json_parser import get_project_root, parse_json_with_fallback
from infrastructure.llm.service import LLMService
from domain.test_case.module_hierarchy import ModuleHierarchyDetector
        ...
        for module in modules:
            ...
            if not entry:
                entry = {
                    ...
                    "keywords": [],
                    "section_hint": (module.get("section_hint") or "").strip() or (canonical_name or raw_name),
                    "exact_phrases": [],
                ...
                if abs(entry["__position"] - module_position) <= ExtractionConfig.MODULE_POSITION_TOLERANCE:
                    # 可能是重复模块，保留描述更详细的
                    new_desc = (module.get("description") or "").strip()
                ...
                if not entry.get("section_hint"):
                    entry["section_hint"] = (module.get("section_hint") or "").strip() or (canonical_name or raw_name)
